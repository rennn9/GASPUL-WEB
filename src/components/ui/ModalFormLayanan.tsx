...
import { getNomorRegistrasi, submitFormLayanan, getStandarPelayanan } from "@/lib/apiLayanan";
...
const ModalFormLayanan = ({ bidang, layanan, onBack }: ModalFormProps) => {
  ...
  // Standar Pelayanan
  const [standarFileUrl, setStandarFileUrl] = useState<string | null>(null);
  ...
  // ===========================
  // Fetch nomor registrasi, persyaratan & standar pelayanan
  // ===========================
useEffect(() => {
  const data = persyaratanLayanan[bidang]?.[layanan] || [];
  setPersyaratanText(data.map((item,i)=>`${i+1}. ${item}`).join("\n"));
  ...
  async function fetchStandar() {
    const url = await getStandarPelayanan(bidang, layanan);
    setStandarFileUrl(url);
  }
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-5 text-gray-800">
            ...
            {/* Field tambahan - muncul jika NIK valid */}
            {nikValid && (
            ...
            {/* Standar Pelayanan */}
            <div>
              <label className="text-sm font-medium text-gray-700">Standar Pelayanan Permohonan</label>
              {standarFileUrl ? (
                <a
                  href={standarFileUrl}
                  target="_blank"
                  className="w-full border border-gray-300 rounded-xl p-3 bg-gray-100 block text-blue-600 hover:underline"
                >
                  📄 Lihat File Standar Pelayanan
                </a>
              ) : (
                <div className="w-full border border-gray-300 rounded-xl p-3 bg-gray-100 text-gray-500">
                  Tidak ada standar pelayanan tersedia.
                </div>
              )}
            </div>
            ...
            <div>
              ...
              >
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm">Drop files here or <span className="text-blue-600">click to upload</span></p>
              ...
              {fileError && (
                <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="mt-3 p-3 rounded-xl bg-red-100 text-red-700 border border-red-300 text-sm">
                ...
                <div className="mt-3 bg-gray-100 rounded-xl p-3 text-sm text-gray-700 space-y-2 max-h-60 overflow-y-auto">
                  {files.map((f, i) => (
                    <div key={i} className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => removeFile(i)} className="text-red-500 text-xs hover:underline">Hapus</button>
                    </div>
                  ))}
                </div>
              )}
